---
record_type: source_dependency
id: source:yiyrlsk-mkk-fon-manual
title: MKK Yatirim Fonu Verileri
status: approved
source_kind: manual_inline
requiredness: required
source_uri: ''
local_hint: mkk_fon_verisi dict olarak notebook icinde inline tanimli
description: MKK tarafindan yayimlanan yatirim fonu portfoy degerleri ve gercek kisi
  kirilimi
usage: Yi_Yrlsk notebook'unda yatirim fonu portfoy analizi
theme_ids:
- theme:resident-deposits
- theme:resident-financial-assets
- theme:resident-securities
indicator_ids: []
validation_note: ''
body: '# MKK Yatirim Fonu Verileri


  ## Kaynak Turu

  manual_inline


  ## Zorunluluk

  required


  ## Kaynak URI

  -


  ## Local Hint

  mkk_fon_verisi dict olarak notebook icinde inline tanimli


  ## Aciklama

  MKK tarafindan yayimlanan yatirim fonu portfoy degerleri ve gercek kisi kirilimi


  ## Kullanim

  Yi_Yrlsk notebook''unda yatirim fonu portfoy analizi


  ## Dogrulama Notu

  -

  '
path: C:\Users\bthkr\OneDrive\Masaüstü\İş Kodlama\İş Agentı\registry\source_dependencies\source%3Ayiyrlsk-mkk-fon-manual.md
---
# MKK Yatirim Fonu Verileri

## Kaynak Turu
manual_inline

## Zorunluluk
required

## Kaynak URI
-

## Local Hint
mkk_fon_verisi dict olarak notebook icinde inline tanimli

## Aciklama
MKK tarafindan yayimlanan yatirim fonu portfoy degerleri ve gercek kisi kirilimi

## Kullanim
Yi_Yrlsk notebook'unda yatirim fonu portfoy analizi

## Dogrulama Notu
-
