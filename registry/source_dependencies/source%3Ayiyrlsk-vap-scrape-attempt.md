---
record_type: source_dependency
id: source:yiyrlsk-vap-scrape-attempt
title: VAP/MKK Scrape Denemesi
status: approved
source_kind: web_scrape
requiredness: optional
source_uri: ''
local_hint: ''
description: Playwright tabanli VAP/MKK iframe scrape denemesi; stabil uretim girdisi
  degil
usage: Yi_Yrlsk notebook'unda manuel ledger yerine gelecek aday kaynak
theme_ids:
- theme:resident-deposits
- theme:resident-financial-assets
- theme:resident-securities
indicator_ids: []
validation_note: ''
body: '# VAP/MKK Scrape Denemesi


  ## Kaynak Turu

  web_scrape


  ## Zorunluluk

  optional


  ## Kaynak URI

  -


  ## Local Hint

  -


  ## Aciklama

  Playwright tabanli VAP/MKK iframe scrape denemesi; stabil uretim girdisi degil


  ## Kullanim

  Yi_Yrlsk notebook''unda manuel ledger yerine gelecek aday kaynak


  ## Dogrulama Notu

  -

  '
path: C:\Users\bthkr\OneDrive\Masaüstü\İş Kodlama\İş Agentı\registry\source_dependencies\source%3Ayiyrlsk-vap-scrape-attempt.md
---
# VAP/MKK Scrape Denemesi

## Kaynak Turu
web_scrape

## Zorunluluk
optional

## Kaynak URI
-

## Local Hint
-

## Aciklama
Playwright tabanli VAP/MKK iframe scrape denemesi; stabil uretim girdisi degil

## Kullanim
Yi_Yrlsk notebook'unda manuel ledger yerine gelecek aday kaynak

## Dogrulama Notu
-
