---
record_type: source_dependency
id: source:yiyrlsk-mkk-hisse-manual
title: MKK Hisse Senedi Yatirimci Verileri
status: approved
source_kind: manual_inline
requiredness: required
source_uri: ''
local_hint: mkk_hisse_verisi ve mkk_hisse_verisi_aylik dict olarak notebook icinde
  inline tanimli
description: MKK tarafindan yayimlanan hisse senedi yatirimci sayilari ve portfoy
  degerleri
usage: Yi_Yrlsk notebook'unda yerli hisse portfoy degeri ve gercek kisi kirilimi
theme_ids:
- theme:resident-deposits
- theme:resident-financial-assets
- theme:resident-securities
indicator_ids: []
validation_note: ''
body: '# MKK Hisse Senedi Yatirimci Verileri


  ## Kaynak Turu

  manual_inline


  ## Zorunluluk

  required


  ## Kaynak URI

  -


  ## Local Hint

  mkk_hisse_verisi ve mkk_hisse_verisi_aylik dict olarak notebook icinde inline tanimli


  ## Aciklama

  MKK tarafindan yayimlanan hisse senedi yatirimci sayilari ve portfoy degerleri


  ## Kullanim

  Yi_Yrlsk notebook''unda yerli hisse portfoy degeri ve gercek kisi kirilimi


  ## Dogrulama Notu

  -

  '
path: C:\Users\bthkr\OneDrive\Masaüstü\İş Kodlama\İş Agentı\registry\source_dependencies\source%3Ayiyrlsk-mkk-hisse-manual.md
---
# MKK Hisse Senedi Yatirimci Verileri

## Kaynak Turu
manual_inline

## Zorunluluk
required

## Kaynak URI
-

## Local Hint
mkk_hisse_verisi ve mkk_hisse_verisi_aylik dict olarak notebook icinde inline tanimli

## Aciklama
MKK tarafindan yayimlanan hisse senedi yatirimci sayilari ve portfoy degerleri

## Kullanim
Yi_Yrlsk notebook'unda yerli hisse portfoy degeri ve gercek kisi kirilimi

## Dogrulama Notu
-
